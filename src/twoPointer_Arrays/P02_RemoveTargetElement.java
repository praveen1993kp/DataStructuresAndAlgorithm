...
public class P02_RemoveTargetElement {
	...
	public void positive(){
		 ...
		 Assert.assertTrue(Arrays.equals(removeMatchingElement(nums,target), output));
	...
	private int[] removeMatchingElement(int[] nums, int target) {
		int pointer1 = 0; //O[1]
		int pointer2 = 0; //O[1]
		while(pointer2<nums.length) { //O[n]
			if(nums[pointer2] != target) { //O[1]
				nums[pointer1++] = nums[pointer2++]; //O[1]
			} else pointer2++; //O[1]
		}
		System.out.println(Arrays.toString(Arrays.copyOf(nums, pointer1)));
		return Arrays.copyOf(nums, pointer1);
	}
